        ...
        <div className="p-4 sm:p-6 overflow-y-auto flex-1 space-y-5">
          {bookedToken ? (
            ...
            <div className="space-y-4">
              ...
              {/* Cards Grid */}
              {/* Statutory Explainability Notice (Requirement 12) */}
              <div className="p-3 bg-emerald-50 rounded-2xl border border-emerald-200 text-[11px] text-emerald-950 flex items-start gap-2">
                <Sparkles className="w-4 h-4 text-emerald-600 shrink-0 mt-0.5" />
                <span>
                  <strong>Doctor Matching Transparency:</strong> Recommendations are shortlisted based on specialty, public health facility schedule, and your selected care requirement — not autonomous medical diagnosis.
                </span>
              </div>

              {/* Cards Grid: Recommended Doctors first, then Other Available Doctors */}
              <div className="space-y-4">
                {/* 1. Recommended Doctors Section */}
                <div className="space-y-2.5">
                  <div className="flex items-center gap-2">
                    <span className="bg-emerald-700 text-white text-[10px] font-black uppercase px-2.5 py-0.5 rounded-full">
                      Recommended Doctors
                    </span>
                    <span className="text-xs text-slate-500 font-semibold">Matched to Care Requirement</span>
                  </div>

                  {filteredDoctors.slice(0, 1).map((doc) => (
                    <div
                      key={doc.id}
                      className="bg-emerald-50/40 rounded-2xl border-2 border-emerald-500 p-4 transition-all shadow-md flex flex-col md:flex-row md:items-center justify-between gap-4"
                    >
                      <div className="flex items-start gap-3.5">
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="bg-emerald-600 text-white text-[10px] font-extrabold px-2 py-0.5 rounded-full flex items-center gap-0.5">
                ...
                {/* 2. Other Available Doctors Section */}
                {filteredDoctors.length > 1 && (
                  <div className="space-y-2.5 pt-2">
                    ...
                    {filteredDoctors.slice(1).map((doc) => (
                      <div
                        key={doc.id}
                        className="bg-white rounded-2xl border border-slate-200 hover:border-slate-300 p-4 transition-all shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4"
                      >
                        <div className="flex items-start gap-3.5">
                          <img
                            src={doc.avatar}
                          ...
                          <div className="space-y-1">
                            <div className="flex items-center gap-2">
                              <h4 className="font-black text-slate-900 text-base">{doc.name}</h4>
                              <span className="bg-slate-100 text-slate-700 text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-0.5">
                        ...
                        <button
                          onClick={() => setSelectedDoctor(doc)}
                          className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-xl transition flex items-center justify-center gap-1.5 shrink-0"
                        >
                          <span>Book</span>
                          <ArrowRight className="w-3.5 h-3.5" />
                        </button>
